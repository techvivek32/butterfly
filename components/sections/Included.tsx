...
/**
 * "What's included with Butterfly" — a dark, glassy comparison table that
 * stacks every replaced tool against a single all-in-one price.
 *
 * Server component: only renders the provided <Reveal*> client wrappers and
 * leans on CSS transitions for the live hover micro-interactions.
 */
      ...
      {items.map((m, i) => (
        <span
          ...
          className={cn(
            ...
            "ring-2 ring-[#072330] shadow-sm transition-transform duration-300",
      ...
      <Reveal direction="up" amount={0.15}>
        <div
          className={cn(
            ...
            "bg-gradient-to-br from-[#0a1410] via-[#072a3a] to-[#06231a]",
            "shadow-float ring-1 ring-white/10",
          )}
        >
          {/* ── ambient backdrop ─────────────────────────────────── */}
          <div
          ...
          <div aria-hidden className="pointer-events-none absolute inset-0 overflow-hidden">
            ...
            <div className="absolute -right-16 top-1/3 h-72 w-72 rounded-full bg-wing-sky/15 blur-3xl" />
            <div className="absolute bottom-0 left-1/2 h-64 w-64 -translate-x-1/2 rounded-full bg-wing-violet/15 blur-3xl" />
          </div>
          {/* top hairline sheen */}
          <div
            aria-hidden
            className="pointer-events-none absolute inset-x-10 top-0 h-px bg-gradient-to-r from-transparent via-brand-400/60 to-transparent"
          />

          <div className="relative">
            {/* ── heading ─────────────────────────────────────────── */}
            <div className="mx-auto max-w-2xl text-center">
              <span className="chip glass-dark mx-auto inline-flex items-center gap-1.5 border-white/10 text-[11px] font-semibold uppercase tracking-[0.14em] text-white/70">
            ...
            {/* ── table card ──────────────────────────────────────── */}
            <div className="mx-auto mt-9 max-w-4xl rounded-2xl border border-white/10 bg-white/[0.03] p-4 backdrop-blur-sm sm:mt-12 sm:p-6">
              {/* header row */}
              <div className="grid grid-cols-[2fr,1fr,auto] items-center gap-4 px-2 pb-3 text-[10px] font-semibold uppercase tracking-[0.12em] text-white/50 sm:grid-cols-[2fr,1fr,1fr,auto] sm:text-xs">
                <div>Features</div>
                  ...
                  <RevealItem key={row.feature}>
                    <div
                      className={cn(
                        ...
                        "border-t border-white/10 transition-colors duration-300 hover:bg-white/[0.05]",
                        "sm:grid-cols-[2fr,1fr,1fr,auto]",
                      )}
                    >
                      {/* feature name */}
                      <div className="flex items-center gap-2.5">
                        <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-brand-gradient shadow-glow transition-transform duration-300 group-hover:scale-150" />
                      ...
                      {/* other tools price */}
                      <div className="text-right text-sm text-white/60 sm:text-left">
                        <span className="tabular-nums line-through decoration-rose-400/40">
                          ${row.price}
                        </span>
              ...
              <Reveal direction="up" delay={0.05} amount={0.3}>
                <div
                  className={cn(
                    ...
                    "border border-white/10 bg-gradient-to-r from-white/[0.06] via-white/[0.03] to-transparent",
                  )}
                >
                  <div className="text-center sm:text-left">
                    ...
                    <div className="mt-1 flex items-baseline gap-1.5">
                      <span className="font-display text-xl font-bold text-white/60 line-through decoration-rose-400/50 decoration-2">
                        {INCLUDED_TOTAL}
                      </span>
            ...
            <Reveal direction="up" delay={0.1} amount={0.4}>
              <div className="mt-9 flex flex-col items-center gap-3 sm:mt-10">
                <a
                  href="#pricing"
                  className={cn(
                    "group inline-flex items-center gap-2 rounded-full bg-white px-6 py-3",
                    "font-semibold text-brand-700 shadow-float ring-1 ring-black/5",
